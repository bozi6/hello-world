...
def tesztkeszlet():
    ...
    teszt(
        szovegbol_szavak('"Nem, én soha!", mondta Alice.')
        == ["nem", "én", "soha", "mondta", "alice"]
    )
    teszt(
        szomszedos_dupl_eltavolit([1, 2, 3, 3, 3, 3, 5, 6, 6, 9, 9, 7])
        == [1, 2, 3, 5, 6, 9, 7]
    )
    teszt(szomszedos_dupl_eltavolit([]) == [])
    teszt(
        szomszedos_dupl_eltavolit(["egy", "kis", "kis", "kölyök", "kutya"])
    ...
    teszt(
        osszefesul(["cica", "egér", "kutya"], ["cica", "kakas", "medve"])
...
def main():
    ...
    print(
        "A könyvben {} szó található, az első 10 a következő:\n{}".format(
            len(konyv_szavai), konyv_szavai[:10]
        )
    )
    print(
        f"A szókincsben {len(nagyobb_szokincs)} szó található, kezdve: \n{nagyobb_szokincs[:6]}"
    )
    ...
    print(
        "A könyvben {} szó van. Csak {} egyedi.".format(
            len(osszes_szo), len(konyv_szavai)
        )
    )
