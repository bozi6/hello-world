...
class TestFuggveny(unittest.TestCase):
    ...
    def test_ismeretlen_szavak_keresese(self):
        self.assertNotEqual(
            ismeretlen_szavak_keresese(szokincs, konyv_szavai),
            ["alice", "s", "adventures"],
        )
...
def main():
    ...
    print(
        "A könyvben {0} szó található, az első 10 a következő:\n{1}".format(
            len(konyv_szavai), konyv_szavai[:10]
        )
    )

    szavak_a_konyvbol("alice_csodaorszagban.txt")
