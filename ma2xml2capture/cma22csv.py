...
if __name__ == "__main__":
    from ma2xml2capture.lampak import lampa as lampak
    ...
    namespace = {
        "bas": "http://schemas.malighting.de/grandma2/xml/MA",
    ...
    print(
        "MA2 programverzió: {},{},{}".format(
            myroot.attrib["major_vers"],
            myroot.attrib["minor_vers"],
            myroot.attrib["stream_vers"],
        )
    )
    ...
    for Layer in myroot.findall("bas:Layer", namespace):
        ...
        for Fixture in Layer:  # Lámpákon szaladunk végig.
            ...
            if (
                onefixture.Patch != "0"
            ):  # ha nincs dmx címe a cuccnak akkor az előzőt adjuk a mdmxnek
                mdmx = onefixture.Patch
            fpos = Fixture[1][1][
                0
            ].attrib  # Fixture/subfixture/absoluteposition/attribjai
            ...
            if (
                "fixture_id" in Fixture.attrib
            ):  # Ha robotlámpa akkor ma2 szerint fixture
